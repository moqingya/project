...
public class Server {
    ...
    public static void main(String[] args) throws IOException {
        ServerSocket serverSocket = new ServerSocket(9999);//服务器端口
        Socket client;
        for (int i = 0; i < 12; i++) {
            rooms[i] = new Room();
        }
...
class PlayerMsg extends Thread {//一个玩家线程,用于选择房间到开始游戏之前的阶段
    BackPlayer player;//一个玩家
    Room[] rooms;//所有房间
    Room room;//选择的房间

    public PlayerMsg(Socket client) {
        this.rooms = Server.rooms;
        this.player = new BackPlayer(client);
    }
    public PlayerMsg(BackPlayer player, Room room){
        this.player = player;
        this.rooms = Server.rooms;
        this.room = room;
    }
...
class GameThread extends Thread {//一局游戏的多线程类
    ...
    ArrayList<BackPlayer> players;

    public GameThread(ArrayList<BackPlayer> players) {
    ...
    public void run() {
        ...
        for (BackPlayer player: players){//游戏结束后,重新开三个玩家交流线程,依靠isInGame的flag跳过选择房间阶段,
