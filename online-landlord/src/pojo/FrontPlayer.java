...
import src.view.RoomWindow;
import src.view.SelectRoomWindow;
import src.view.StartWindow;
...
//            String ip = "106.13.40.227";//服务器ip
            ...
            InetSocketAddress socketAddress = new InetSocketAddress(ip, 9999);
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     */
    //游戏局内
    public void playGame(MusicPlay mc) throws IOException, InterruptedException {
        do {
    ...
    public void gameRound() throws IOException, InterruptedException {
        MusicPlay mc = new MusicPlay();
        mc.MusicPlay("res\\music\\fightmusic.wav");
        mc.musicMain(3);
        while (true) {
            serverMessage = in.readUTF();
            if (serverMessage.equals("请你出牌")) {
                ...
                if (serverMessage.equals(whoIsLord)) {
                    if (isLord) {
                        ...
                        MusicPlay mcWin = new MusicPlay();
                        mcWin.MusicPlay("res\\music\\win.wav");
                        mcWin.musicMain(1);
                        gameWindow.showLordWin();
                    } else {
                        System.out.println("农民输了");
                        MusicPlay mcLose = new MusicPlay();
                        mcLose.MusicPlay("res\\music\\lose.wav");
                        mcLose.musicMain(1);
                        gameWindow.showLordWin();
